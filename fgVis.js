document.addEventListener('DOMContentLoaded', function() {
  ...
  function createFGPercentageVisualization(data) {
    ...
    let sortedData = [...otherPlayers, ...lastSixPlayers];
    ...
    vizCard.append('p')
      ...
      .html('<i class="fas fa-arrows-alt me-1"></i>Click & Drag players for easier comparison!');
    ...
    const playerGroups = svg.selectAll('.player-group')
      .data(sortedData, d => d["Player Name"])
      .enter()
      .append('g')
      .attr('class', 'player-group')
      .attr('id', d => `player-${d["Player Name"].replace(/\s+/g, '-')}`)
    ...
    legend.append('text')
      ...
      //needed for the dragging feature
    function updateChart() {
      xScale.domain(sortedData.map(d => d["Player Name"]));
    ...
    const drag = d3.drag()
      ...
      .on('end', function(event, d) {
        ...
        const playerNames = sortedData.map(p => p["Player Name"]);
        const currentIndex = playerNames.indexOf(playerName);
        
        // find index with other bars
        let targetIndex = playerNames.findIndex(name => {
          return draggedX < xScale(name) + xScale.bandwidth() / 2;
        });
        
        // edge cases
        if (targetIndex === -1) {
          targetIndex = playerNames.length - 1;
        }
        
        // update array for order after move
        if (currentIndex !== targetIndex) {
          const player = sortedData.splice(currentIndex, 1)[0];
          sortedData.splice(targetIndex, 0, player);
          ...
          d3.select(this)
            .transition()
            .duration(300)
            .attr('transform', `translate(${xScale(playerName)}, 0)`);
        }
      });
    
    playerGroups.call(drag);
    
    vizCard.append('button')
      .attr('class', 'btn btn-outline-secondary mt-4 mb-2')
      .html('<i class="fas fa-undo me-1"></i>Reset Order')
      .on('click', function() {
        sortedData = [...otherPlayers, ...lastSixPlayers];
        updateChart();
      });
  }
});